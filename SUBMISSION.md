# Hazir - Prototype Phase submission
...
**YOU.** Pick your Idea Phase submission for Hazir from the dropdown.
...
```
2. Smart Cities, Urban Safety & Mega-Project Infrastructure
```
...
1. The dashboard after Run all eight site events, showing the batch table and the cost comparison.
...
8 CAMARA APIs, 77.5% cheaper than calling every check.
...
```
https://github.com/mert-gng-99/hazir-nac-agent
```
...
| --- | --- | --- |
| Parent Submission | Pick Hazir from the dropdown | seconds |
...
| --- | --- |
| At least one CAMARA API on Nokia NaC | 8 API families: `number-verification`, `location-verification`, `device-status`, `location-retrieval`, `congestion-insights`, `geofencing-subscriptions`, `quality-on-demand`, `network-slice-device-attachment` |
...
| Solves a real problem | Smart Cities, Urban Safety & Mega-Project Infrastructure. Stated limits are in README.md, so this is not pitched as doing more than it does. |
| Theme alignment | Theme 2 |
| Multiple CAMARA APIs (good-to-have) | 8 families orchestrated per case |
| Intelligent orchestration (good-to-have) | 1 to 6 calls depending on the case; 77.5% cheaper than calling everything |
