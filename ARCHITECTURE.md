...
For Hazir: at induction, alongside the site pass and the safety briefing, from the worker, who is a contracted employee and on most Gulf sites already carries the company line as part of the kit. Checks run during shift hours and during a declared incident, and stop when the worker clocks out. Zone level only, never a pin.
